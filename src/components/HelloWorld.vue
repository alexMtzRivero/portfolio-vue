<template>
  <div class="main" v-on:click="nextText()">
    <h1 v-for="text in shown" :key="text">
      <span>me@terminal:-$</span> {{ text }}
    </h1>
    <div v-if="index == text.length - 1">
      <router-link to="/CV"> My Cv </router-link>
      <a
        target="_blank"
        href="https://linkedin.com/in/alejandro-martinez-rivero-116836120"
      >
        LinkedIn
      </a>
  ...
  methods: {
    ...
    typeText: function (text, speed) {
      if (text.length > 0 && this.writing) {
        ...
        setTimeout(() => {
          this.typeText(text.slice(1, text.length), speed);
